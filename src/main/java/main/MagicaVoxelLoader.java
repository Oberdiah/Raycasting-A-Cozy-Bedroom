...
public class MagicaVoxelLoader {
	...
	public void read(InputStream input, Callback callback) throws IOException {
		...
		Queue<Integer> queueInt = new ArrayDeque<>();
		Queue<Byte> queueByte = new ArrayDeque<>();
		...
		while (true) {
			...
			if (chunk.id == magicValue('S', 'I', 'Z', 'E')) {
				...
				for (int v = 0; v < numVoxels; v++) {
					queueInt.add(input.read());
					queueInt.add(input.read());
					queueInt.add(input.read());
					queueByte.add((byte) (input.read() & 0xff));
				}
		...
		if (!foundPalette) {
			System.out.println("Didn't find palette! Reverting to default materials.");
		...
		while (!queueInt.isEmpty()) {
			callback.voxel(queueInt.remove(), queueInt.remove(), queueInt.remove(), queueByte.remove());
		}
	}
